...
public class RecallPotionItem extends SimplePotionItemWithCooldown {
    public RecallPotionItem(int cooldownDuration, int drinkingDuration, Properties pProperties) {
        super(MobEffects.CONFUSION, 0, 0, ModEffects.HEALING_SICKNESS, cooldownDuration, drinkingDuration, "Recall", pProperties);
    }
        ...
        if (pEntityLiving instanceof ServerPlayer player) {
            if (!pLevel.isClientSide) {
                ...
                if (respawnPoint != null) {
                    DimensionTransition respawnPosition = player.findRespawnPositionAndUseSpawnBlock(false, DimensionTransition.DO_NOTHING);
                    player.changeDimension(respawnPosition);
                }
            }
        }
    }
